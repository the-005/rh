...
- `index.tsx` — `ProjectPage`, on white, over the canvas. Phases `arrive | hero | leaving`.
  ...
  - **Exit, from any image** (× or Escape; clicks are navigation). Decided by the owner: "the user can exit from any image, we will just change the image on the homepage that they come from to the image that they closed out with."
    ...
    - Falls back to a plain fade if the texture doesn't arrive within `SWAP_TIMEOUT_MS` (1.2s), and on Escape mid-move. (Deep links to `/project/:id` have no plane either and fade in and out.)
...
**`src/about/index.tsx`** — `AboutPage`: the third view, at `/about`, on the same white as the index, at `z-index: 100` under the frame. **Currently empty**: all placeholder copy (statement, contact, studio, services, clients) was removed at the owner's request, pending real text. The old two-column layout is in git history (before this change).
...
**`src/index-page/index.tsx`** — `IndexPage`: the same projects as a list, on white. **Full-width rows of title + year**, with no header row and no number column. Hovering a row dims the others (the row doesn't move) and shows its images **centred on screen at 66% of viewport height**. **Each row is a player.** Cursor x scrubs to an image (the preview follows after `COMMIT_MS`). A grey fill (`rgba(17,17,17,0.07)`) covers the row from its left edge to the centre of the current image's cell, a progress bar. It ends at the centre of the **playhead** sitting on the row's line: a **square while stopped, a triangle while playing**. **Clicking the row toggles a slideshow** from the current image (`PLAY_MS` 3000, looping; 1.5s was too fast). While it plays the cursor doesn't scrub; click again to stop and scrub again. Leaving the row stops it. **Motion:** the bar never snaps. Scrubbing glides (`GLIDE`, 350ms ease-out). Playing runs at constant speed: during each image the bar goes linearly from that image's centre to the next's, arriving as the next image comes up, and it jumps back (never glides backwards) when the loop wraps. **Image changes crossfade**, after estudioalem.com, whose loop is Swiper 6's `fade` effect (`speed: 4000`, ease-out, autoplay `delay: 2000`, `crossFade: false`): images stacked in one place, the incoming fading in over the outgoing. Layers stack in a one-cell grid, with the timing in CSS. **While playing: a long 2s crossfade that keeps the double exposure.** The new image comes up on `ease` while the old holds on an ease-in, both starting together, so at least one is always ≥ 84% and the frame never lifts toward white. (A short 0.7s version read as mud. A no-overlap dissolve fixed that but flashed white between images, and the owner preferred the long double exposure.) **While scrubbing:** a quick 0.25s dissolve that keeps up with the cursor. Layers are capped at `MAX_LAYERS` and swept after `FADE_CLEANUP_MS`. **The index doesn't open project pages** (removed at the owner's request; `setPendingIndex` went with it). **Layering, by request:** the hovered row's name and year are on top (`z-index: 2` on `.title`/`.year`), then the preview (`1`), then everything else: other rows, and the hovered row's bar, playhead and lines. `.row` and `.inner` deliberately set no z-index, so these interleave in the page's stacking context. It lives at `/index` (a route, so it survives a reload and the back button) at `z-index: 100` under the frame. Touch and narrow screens hide the preview and player.
