...
**`src/research/index.tsx`** — `ResearchPage`: the Research images as a grid, at `/research`, on the same white as the index and about. Built from the Codrops grid-layout-transition demo (MIT), with its numbering and its GSAP FLIP tween removed. A scale step (50–150%, centred in the frame's top row) just changes the column count (16/10/8/6/4; fewer on tablet; always 3 on phones, where the steps are hidden), so the new layout lands in a single frame, the way Eagle's zoom does; tweening tiles between cells read as lag. Images keep their own aspect ratios and **hang from the top of each row** (`align-items: start`), not centred. It reads `src/research/manifest.json` (see Research media).
