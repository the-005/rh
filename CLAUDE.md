...
**Depth cycling (scroll zoom effect)**: Scrolling drives `velocity.z` applied to each image's `absoluteZOffset` rather than moving the camera. `effectiveZ = INITIAL_CAMERA_Z - zOffset`. Each plane has a **fixed** direction `zDir` (checkerboard parity of `cx + cy + slot`): half zoom in on scroll-up, half zoom out. Direction must never be camera-relative — a left/right-of-camera split lets panning flip directions mid-flight, which scrambles relative depth phases and can permanently weld two planes to the same depth. Opacity fade zones: `depthFadeNear` (fade in), flat visible plateau, `depthFadeStart` → `depthFadeEnd` (fade out + wrap back to 0).

**Depth phase assignment** (`PlaneData.depthPhase`): Pre-computed in `generateChunkPlanes`. All planes in an XY chunk column (3 z-layers × `itemsPerChunk`) are stratified onto evenly spaced depth slots (`zSpread / (3 × itemsPerChunk)` apart — 150 units at defaults). Each column's base phase is `frac(cx·ALPHA_X + cy·ALPHA_Y + sessionFrac)`; the constants are grid-search-optimized over **same-direction** pairs only (opposite-direction pairs drift on scroll, so only same-parity pairs keep a fixed gap) — every persistent pair within ±2 columns stays ≥31.8 units apart, with a ±4-window guard against exact rational welds. Beware: constants in an exact rational ratio (e.g. 2:3) lock entire column sublattices to identical phases.
