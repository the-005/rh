...
- `index.tsx` — `ProjectPage`: all of a project's images in one centered row, equal height, scaled to fit the viewport width (32px side margins, 4px gaps, height capped at 50vh), on a white background matching the homepage. The clicked image is rotated to be first (leftmost). No slider/nav; click anywhere or Escape closes.
