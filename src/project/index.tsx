...
export function ProjectPage() {
  ...
  // Capture once — consumeTransitionOrigin() clears the module-level store
  const originRef = React.useRef(consumeTransitionOrigin());

  // FLIP entry: expand from click origin
  ...
  return (
    ...
    <div className={styles.overlay} ref={overlayRef} onClick={handleClick}>
      <button type="button" className={styles.close} onClick={(e) => { e.stopPropagation(); handleClose(); }}>
        ×
      </button>

      <div className={styles.stage}>
        {images.map((img, i) => {
          // Signed offset from current with wrap-around
